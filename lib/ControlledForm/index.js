...
export default class ControlledForm extends Component {
  constructor(props) {
    ...
    _defineProperty(this, "validateForm", (isShowError = false, callback, data) => {
    ...
    _defineProperty(this, "validate", (dataIndex, isShowError = true, callback) => {
    ...
    _defineProperty(this, "onCheckFormValidate", () => {
      // console.log('!!!!!----')
      let items = this.items.filter(x => x !== null && x !== undefined);
  ...
  componentWillMount() {
    ...
    this.props.handles && this.props.handles({
  ...
  // 控件数据变更
  onItemChange(key, value, ...arg) {
    let data = clone(this.props.value);

    _.set(data, key, value);

    this.props.onChange && this.props.onChange(data, key, value, arg);
  }
  ...
  render() {
    ...
    if (this.props.isChildForm) {
      let className = (this.props.className || '') + this.props.layout ? ` ant-form-${this.props.layout}` : '';
...
_defineProperty(ControlledForm, "defaultProps", {
  layout: 'horizontal',
  // 表单布局 'horizontal'|'vertical'|'inline' 参照antd
  className: '',
  onSubmit: null,
  // 提交事件，进行校验
  onChange: null,
  ...
  handles: null,
  // 向外部暴露接口
  isChildForm: false // 是否为子表单，兼容表单套表单
...
class Item extends Component {
  constructor(_props) {
    ...
    _defineProperty(this, "checkValidateCallback", (result, isShowError = true, callback) => {
    ...
    _defineProperty(this, "checkValidate", (value, isShowError = true, callback) => {
      let result = {
    ...
    _defineProperty(this, "onChange", (e, ...arg) => {
      let value = getValueFromEvent(e);
      this.inputValue = value;
      this.props.emitChange(this.props.dataIndex, value, e, ...arg);
    });

    _defineProperty(this, "onStashChange", e => {
      let value = getValueFromEvent(e);
      this.setState({
        stashValue: value
      });
    ...
    _defineProperty(this, "renderDecorator", element => {
      if (!element) {
        return null;
      }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.value !== this.props.value) {
      let showError = false; // @todo 确定是否会存在bug，当前判断逻辑：判断本次数据和刷新前用户输入数据是否一致，如果一致，则判定为需要显示错误框；否则不显示错误信息
  ...
  render() {
    ...
    if (this.props.rules && _.isArray(this.props.rules)) {
    ...
    return React.createElement(FormItem, _extends({}, this.props.itemProps, this.props, {
      required: required,
      help: this.state.help,
      validateStatus: this.state.validateStatus
    }), decorator, this.props.children);
  }
