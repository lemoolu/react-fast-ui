...
export default class Fetch extends PureComponent {
  constructor(props) {
    ...
    _defineProperty(this, "setCacheData", data => {
      let key = this.getCacheKey();
      window.sessionStorage.setItem(key, JSON.stringify(data));
    });

    _defineProperty(this, "getData", async history => {
      // 本地有缓存的情况是使用缓存
      if (this.props.cache) {
        // 数据获取中，定时轮询获取数据
...
export function withFetch(WrappedComponent) {
  return class extends React.Component {
    render() {
      ...
      if (fetch) {
        return React.createElement(Fetch, fetch, React.createElement(WrappedComponent, otherProps));
      }
